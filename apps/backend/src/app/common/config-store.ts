 ...
 * the not-yet-saved case, save as idempotent upsert. Used by `app_settings` -
 * the one remaining config that is genuinely global rather than per-device
 * (wallbox/SMA moved to `device_config`, addressed by id via
 * DeviceConfigService, once there could be more than one of each).
 */
